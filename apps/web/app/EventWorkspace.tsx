...
export function EventWorkspace({ onOpenDocument }: { onOpenDocument: (documentId: string) => void }): ReactElement {
  ...
  return <section className="panel">
    ...
    <section className="review-queue"><strong>Review queue</strong>{reviews.length === 0 ? <p className="muted">Nessuna revisione aperta.</p> : reviews.map(review => { const candidateId = typeof review.context.candidate_document_id === "string" ? review.context.candidate_document_id : ""; const evidence = Array.isArray(review.context.evidence) ? review.context.evidence.filter((item): item is string => typeof item === "string") : []; const conflicts = Array.isArray(review.context.conflicts) ? review.context.conflicts.filter((item): item is string => typeof item === "string") : []; const identityEvidence = Array.isArray(review.context.resolution_evidence) ? review.context.resolution_evidence.filter((item): item is string => typeof item === "string") : []; const score = typeof review.context.score === "number" ? review.context.score : 0; const sourceAvailable = documents.some(document => document.id === review.entity_id); const candidateAvailable = documents.some(document => document.id === candidateId); const isIdentityConflict = review.type === "PATIENT_CONFLICT"; const canResolveLink = sourceAvailable && candidateAvailable; return <div key={review.id} className="review-item"><div className="review-copy"><strong>{isIdentityConflict ? "Identità paziente da verificare" : "Collegamento proposto"}</strong>{isIdentityConflict ? <><p><button className="review-document-link" disabled={!sourceAvailable} onClick={() => onOpenDocument(review.entity_id)}>{documentLabel(review.entity_id)}</button></p><p>Il nome ha un solo riscontro in famiglia, ma il codice fiscale stampato non coincide.</p><div className="review-signals">{identityEvidence.map(item => <span className={`review-signal${item === "unmatched_fiscal_code" ? " conflict" : ""}`} key={item}>{item === "reversed_full_name" ? "Nome e cognome invertiti" : item === "unmatched_fiscal_code" ? "Codice fiscale discordante" : item}</span>)}</div></> : <><p><button className="review-document-link" disabled={!sourceAvailable} onClick={() => onOpenDocument(review.entity_id)}>{documentLabel(review.entity_id)}</button> ↔ <button className="review-document-link" disabled={!candidateAvailable} onClick={() => onOpenDocument(candidateId)}>{documentLabel(candidateId)}</button></p><p>Apri entrambi i documenti e verifica le evidenze prima di decidere.</p><p>Confidenza {Math.round(score * 100)}% · priorità {review.priority}</p><div className="review-signals">{evidence.map(item => <span className="review-signal" key={item}>{signalLabel(item)}</span>)}{conflicts.map(item => <span className="review-signal conflict" key={item}>{signalLabel(item)}</span>)}</div></>}</div><div className="review-actions">{!isIdentityConflict && <button className="text-button" disabled={!canResolveLink} onClick={() => { if (window.confirm("Creare un episodio di cura con questi due documenti?")) void resolve(review, "confirmed_related"); }}>Crea episodio</button>}<button className={`text-button${isIdentityConflict ? "" : " reject"}`} disabled={!isIdentityConflict && !canResolveLink} onClick={() => void resolve(review, isIdentityConflict ? "acknowledged_identity_conflict" : "rejected_unrelated")}>{isIdentityConflict ? "Conferma identità" : "Non sono collegati"}</button></div></div>; })}</section>
  </section>;
}
