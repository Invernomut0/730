...
export function EventWorkspace({ onOpenDocument }: { onOpenDocument: (documentId: string) => void }): ReactElement {
  ...
  return <section className="panel">
    ...
    {!selectedId ? <p className="muted">Un evento compare qui dopo un collegamento verificato tra prescrizione e fattura.</p> : <div className="workspace-grid">
      ...
      <aside className="workspace-aside"><strong>Assicurazione</strong>{evaluation && <><p>{evaluation.status === "candidate" ? "Candidata al rimborso" : "Da verificare"}</p><p>Documentazione {evaluation.documentation_complete ? "apparentemente completa" : "incompleta"}</p><p>Stima: {evaluation.warnings.some(item => item.includes("Invoice total is unavailable")) ? "non disponibile" : `€ ${evaluation.estimated_eligible_amount}`}</p>{evaluation.missing_documents.map(item => <p key={item}>Manca: {item}</p>)}{evaluation.warnings.map(item => <p className="muted" key={item}>{item}</p>)}</>}</aside>
    </div>}
