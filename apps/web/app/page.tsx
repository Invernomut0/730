...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <DocumentViewerLayer document={documents.find(item => item.id === selectedDocumentId)} pages={pages} onClose={() => { setSelectedDocumentId(undefined); setPages([]); setSelectedWord(undefined); }} onDocumentUpdated={updated => { setDocuments(items => items.map(item => item.id === updated.id ? { ...item, extraction: updated.extraction } : item)); setMessage("Prestazione fattura aggiornata e collegamenti ricalcolati."); }} />
    <FamilyPanel />
    <Precompiled730Panel />
