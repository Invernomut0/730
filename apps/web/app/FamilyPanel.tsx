...
export function FamilyPanel(): ReactElement {
  ...
  return <section className="panel">
    ...
    <div className="family-list">{households.map(household => <div key={household.id} className="family-card"><strong>{household.name}</strong>{household.members.length === 0 ? <p className="muted">Nessun membro.</p> : <ul>{household.members.map(item => <li key={item.id}>{item.first_name} {item.last_name} · {item.relationship_type ?? "relazione non indicata"}</li>)}</ul>}</div>)}</div>
  </section>;
}
